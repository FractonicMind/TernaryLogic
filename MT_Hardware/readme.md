...
The bilayer stack enables sequential switching: partial RESET ruptures only the TaOx+ filament segment (LRS to IRS), full RESET ruptures both segments (LRS to HRS). Each state corresponds to a distinct physical configuration, not a software label.
...
- Crossbar arrays: 64x64 per hierarchical block maximum (IR drop limit)
- Confirm wire length: 500 um per window comparator instance maximum
- Operating temperature: 0-125°C
- Execution lane WCET: 2 ms maximum at 99.99th percentile
- Logging lane hard ceiling: 300 ms maximum, jitter 50 ms maximum
