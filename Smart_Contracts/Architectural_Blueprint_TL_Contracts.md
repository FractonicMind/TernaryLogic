...
| :---- | :---- | :---- | :---- |
| **Technical Council** | 9 Members (75% Quorum — 7 votes) | Technical Integrity | - Preserve and update core specifications<br>- Approve protocol improvements<br>- Commission security audits |
| **Stewardship Custodians** | 11 Members (75% Quorum — 9 votes) | Ethical & Legal Oversight | - Enforce "No Spy" / "No Weapon" prohibitions<br>- Certify/revoke operator licenses<br>- Arbitrate escalated disputes |
...
The Treasury accumulates TL service fees collected on-chain. Fee levels are governance
parameters set by Tri-Cameral Joint-Approval (Nomination 2026 — subject to revision).
Epistemic Hold activation and resolution carry no fee by constitutional design. Disbursements
to the Goukassian Foundation require Joint-Approval supermajority independently in both
the Technical Council and the Stewardship Custodians.
...
#### 5.1.3. Sequence Diagram: Resolving the `Epistemic Hold,` State via Oracle-Custodian Interaction
...
SC->>SC: Transition to Epistemic Hold(0)  

SC->>L: Log Epistemic Hold State  

SC->>O: Emit EpistemicHoldInitiated Event  

O->>O: Gather Evidence / Deliberate  

O->>SC: resolveEpistemicHold(callback, decision)  
...
#### 5.2.1. Solidity Smart Contract: Core State Machine and `Epistemic Hold()` Logic
...
enum TernaryState { Intent, Epistemic Hold, Commit, Reject }
...
event EpistemicHoldResolved(bytes32 indexed actionId, TernaryState finalState, string reasoning);
...
function initiateAction(bytes32 _actionId, string memory _intent) external {  
    ...
    // Transition to Epistemic Hold for initial verification  

    _transitionToEpistemicHold(_actionId, "Initial verification required");  

}

function _transitionToEpistemicHold(bytes32 _actionId, string memory _reason) internal {  
    ...
    // For this example, we assume the oracle calls resolveEpistemicHold later.  

}

function resolveEpistemicHold(bytes32 _actionId, uint8 _decision, string memory _reasoning) external onlyOracleCustodian nonReentrant {  

    require(decisions[_actionId].state == TernaryState.EpistemicHold, "Not in Epistemic Hold state");  
...
States == {"Intent", "Epistemic Hold", "Commit", "Reject"}
...
TransitionToEpistemic Hold ==  
/ state = "Intent"  
/ state' = "Epistemic Hold"  
/ actionLog' = Append(actionLog, [state |-> "Epistemic Hold", timestamp |-> 1])
...
StateMachineOrder ==  
[]((state = "Intent" => state' in {"Epistemic Hold", "Intent"}) /  
(state = "Epistemic Hold" => state' in {"Commit", "Reject", "Epistemic Hold"}) /  
(state = "Commit" => state' = "Commit") /  
(state = "Reject" => state' = "Reject"))

(* Liveness Property: If in Hold, eventually a final state is reached *)  
EventuallyFinalizes ==  
[](state = "Epistemic Hold" => <>(state in {"Commit", "Reject"}))  
=============================================================================  
```
